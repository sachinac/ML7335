...
def downloadFromUrl( url, targetDir=".", removeSource=False):
    ...
    except  (IOError, zip.BadZipfile) as e:
            os.remove(baseName)
...
dig_dict    = {1 : 'A',2 : 'B', 3 :'C' ,4: 'D',5:'E' }
downloadFromUrl('https://github.com/sachinac/ML7335/blob/main/hwdataset/train.zip?raw=true',removeSource=True)
data_dir = pathlib.Path('train/')

train_img = []
for img in list(data_dir.glob('*.png')):
    letter_c = str(img)
    img = image.load_img(letter_c, target_size=(28, 28))
    img = image.img_to_array(img)
    train_img.append(img)

train_labels = [ os.path.basename(str(images)) for images in list(data_dir.glob('*.png')) ]  
...
downloadFromUrl('https://github.com/sachinac/ML7335/blob/main/hwdataset/test.zip?raw=true',removeSource=True)
...
test_labels = [ os.path.basename(str(images)) for images in list(data_dir.glob('*.png')) ]
...
train_label_all = [ letter_dict[label[0]] for label in train_labels] 
test_label_all = [ letter_dict[label[0]] for label in test_labels] 
...
def Show_example_digits(mono = 'gray'):
    ...
    for idx in range(35):
        ...
        plt.title("Letter {"+dig_dict[ train_label_all[idx] ]+"}")
